...
if (isset($_POST['s'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        $name = $_POST['email'];
        $pass = $_POST['My_password']; // Corrected name attribute
        $_SESSION['u_id'] = $name; // Corrected session variable
...
<body style="background-size: cover; background-image: url(Regi.jpg); background-position: center;">
    ...
    <div  align="center" >
        <div class="form" style="margin-top: 15%">
            <form method="post">
                ...
                <div class="form-group" style="width:30%">
                    <label for="exampleInputPassword1"><h1 style="color: #fff;">My Password</h1></label>
                    <input type="password" class="form-control" id="exampleInputPassword1" placeholder="My Password" required name="My_password" onfocusout="f1()">
                </div>
                <button type="submit" class="btn btn-secondary" name="s" onclick="f1()">Submit</button>
            </form>
        </div>
    </div>
