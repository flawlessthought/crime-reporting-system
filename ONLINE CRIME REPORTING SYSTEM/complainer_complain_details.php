...
<head>
    ...
    if(isset($_POST['assign']))
    {
        if($_SERVER["REQUEST_METHOD"]=="POST")
        {
            ...
            $selectedPolice = $_POST['selectedPolice'];

            // Update complaint with the selected police officer
            $q4 = mysqli_query($conn, "UPDATE complaint SET p_id = '$selectedPolice', inc_status = 'Assigned' WHERE c_id = '$complainid'");
        }
    }
    
    $result=mysqli_query($conn,"select p_id,p_name,spec,location from police where location='$location'");  
    $complaints = mysqli_query($conn, "SELECT c_id, type_crime FROM complaint WHERE inc_status='Unassigned'");
    ?>
   ...
   <a href="police_add.php"><input  type="button" name="add" value="Add Police Officers" class="btn btn-primary"></a>
    ...
    <div style="padding:50px;">
        <form method="post" action="">
            <label for="complainid">Select Complaint:</label>
            <select name="complainid">
                <?php
                while ($complaint = mysqli_fetch_assoc($complaints)) {
                    echo "<option value='{$complaint['c_id']}'>{$complaint['type_crime']}</option>";
                }
                ?>
            </select>
